...
class DecodingAnalysis(Base):
    ...
    def run(self):
        ...
        for scoring_strategy in self.config["scoring_strategy"]:
            pvalues = []
            cohensds = []
            score_mean = []
            time_lst = []
            ...
            for start_time in np.arange(self.config["t_start"], self.config["t_end"], self.config["t_sampling"]):
                ...
                for self.subject in self.config["subjects"]:
                    with open(fname.decoding_score(subject=self.subject), "r") as json_file:
                        ...
                        json_data = json_data[scoring_strategy]
            ...
            figure_decoding_analysis.suptitle(f"Evaluation of decoding technique {scoring_strategy}")
            ...
            self.add_figure(figure_decoding_analysis, caption=f"Strategy {scoring_strategy}: Decoding score, significance and effect size across time")
